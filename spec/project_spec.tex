...
\begin{itemize}
  \item \verb|scripts/synth/NCO.py| - the NCO model, outputs all 4 waveform types
  \item \verb|scripts/synth/Blocks.py| - models of the summer and truncator (with global gain)
  \item \verb|scripts/synth/Synth.py| - model of the full mono and poly synths
\end{itemize}

You should use \verb|Synth.py| to dump golden DAC samples for testing in simulation.
...
You will be \textbf{primarily} graded on trying several design points (tradeoffs between CPI and clock frequency), documenting your results, and achieving the best \verb|mmult| performance.
You must demonstrate a working CPU at an optimized clock frequency (above 50MHz) that has a working BIOS, can load and execute programs (both echo and mmult), can receive, process, and send to user I/O, and has a working audio synthesizer.
...
Scrolling down, there's an instantiation of \verb|PLLE2_ADV|, which is a PLL (phase locked loop) primitive on the FPGA.
This is a circuit that can create a new clock from an existing clock with a user-specified multiply-divide ratio.

The \verb|CLKIN1| input clock of the PLL is driven by the 125 MHz \verb|CLK_125MHZ_FPGA|.
The frequency of \verb|CLKOUT0| is calculated as:
\begin{equation*}
  \mathtt{CLKOUT0}_f = \mathtt{CLKIN1}_f \cdot \frac{\mathtt{CLKFBOUT\_MULT}}{\mathtt{DIVCLK\_DIVIDE} \cdot \mathtt{CLKOUT0\_DIVIDE}}
\end{equation*}

In our case we get:
\begin{equation*}
  \mathtt{CLKOUT0}_f = 125 \text{ MHz} \cdot \frac{34}{5 \cdot 17} = 50 \text{ MHz}
\end{equation*}

Table \ref{pll_params} specifies the ideal PLL parameters to get certain output frequencies.

\begin{table}[hbt]
  \begin{center}
    \caption{PLL Parameters}
    \label{pll_params}
    \begin{tabular}{l l l l}
      \toprule
      Frequency & \verb|DIVCLK_DIVIDE| & \verb|CLKFBOUT_MULT| & \verb|CLKOUT0_DIVIDE|\\
      \midrule
      50 MHz & 5 & 34 & 17 \\
      60 MHz & 5 & 36 & 15 \\
      65 MHz & 5 & 39 & 15 \\
...
The timing tools will automatically figure out the CPU's clock timing constraint based on the multiply-divide ratio you used in your PLL.
...
To visualize the path, you can open Vivado (\verb|make vivado|), and open a DCP (Design Checkpoint) file (\verb|File| $\rightarrow$ \verb|Checkpoint| $\rightarrow$ \verb|Open|).
The DCP is in \verb|build/impl/z1top_routed.dcp|.

Re-run timing analysis with \verb|Reports| $\rightarrow$ \verb|Timing| $\rightarrow$ \verb|Report Timing Summary|.
...
These post-PNR timing reports may be hard to decipher, so it may help to look at the post-synth DCP in \verb|build/synth/z1top.dcp|.

You can also use the \href{https://www.xilinx.com/support/answers/54778.html}{\texttt{keep\_hierarchy} attribute} to prevent Vivado from moving registers and logic across module boundaries.
